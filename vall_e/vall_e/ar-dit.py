...
class AR(nn.Module):
    ...
    def __init__(self, d_model, n_steps, n_tokens, max_n_levels, n_heads, num_layers):
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(1024, 16, mlp_ratio=4.0) for _ in range(8)
        ])

        self.final_layer = FinalLayer(1024, 1024)
    ...
    def reverse_diffusion(self, x_noisy, t, noise, device="cuda"):
        ...
        if(t==100):
            return torch.tensor(2.00, dtype=torch.float16).to(device) * (x_noisy.to(device) - sqrt_one_minus_alphas_cumprod_t.to(device) * noise.to(device))
        # Mean reconstruction
    ...
    def forward(self, text_list, proms_list, resps_list=None):
        ...
        if(resps_emb[0].shape[0]<300):
            resps_emb=[F.pad(resps_emb[0], (0, 0, 0, 300-resps_emb[0].shape[0]))]
        else:
            resps_emb=[resps_emb[0][:300,:]]
        ...
        if resps_list is not None:
            for t in range(1, self.num_steps+1 ):
                ...
                time_emb=self.time_emb([t_tensor])

                x=torch.cat([x_noise,proms_emb[0],text_emb[0],time_emb[0]], dim=0)
                x=self.sin_emb.add_pe(x)[0]
